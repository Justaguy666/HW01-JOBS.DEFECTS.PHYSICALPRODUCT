...
- Confirm each posting date is within the required 60-day window at final submission time.
- Confirm salary information is either captured or explicitly marked unavailable.
...
- Review whether each selected item is a concrete publicized software defect, not only a paper/dataset-level finding.
- Confirm each defect has a reliable public source link and a screenshot.
- Cross-link the 20 defects into the final report.

## Requirement 3 - Physical Product Testing

Current state: partially present.

- Specific physical device evidence files are present, including device photo and packaging/photo evidence.
- 15 test case Markdown files are present.
- 5 discovered defect Markdown files are present.
- 3 AI-missed edge-case explanation files are present.
- 5 GitHub issue URLs are present in Requirement 3 materials:
...
- `execution_videos/youtube_links.txt` now records that no YouTube execution videos are provided.

Remaining checks:

- Video requirement is skipped: no 5 unlisted YouTube links and no voice narration evidence will be provided.
- Screenshot/evidence check is treated as pass per user decision.
